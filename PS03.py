...
T_1 = ufloat(200 + 273.15, 100)  # heiße Temperatur (upper bound 800°C)
T_2 = ufloat(8 + 273.15, 2)  # kalte Temperatur
...
data = [
    ...
    3.3960,
    3.4280,
]  # Flächenintegrale der Kurve (Arbeit) Pa/m^3
dW_B = 0  # Unsicherheit der Arbeit (Flächenintegral)
W = mean_with_uncertainty(data, dW_B)  # Arbeit (FLäche in Kurve, ACHTUNG Einheiten)


# %% 1. idealer Wirkungsgrad
n_ideal = (T_1 - T_2) / T_1
print(f"n_ideal ={n_ideal:.5f}")
...
n_bel = P_motor / P_zu
print(P_motor)
print(f"n_bel ={n_bel:.5f}")


# %% 1. mechanischer Wirkungsgrad
n_motor = P_motor / (
    W * f_gebremst
)  # check das mit den fs nicht, kp was ma da braucchen, stimmt ws?
print(f"n_motor ={n_motor:.5f}")
